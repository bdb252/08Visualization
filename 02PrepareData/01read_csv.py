...
read_csv()
    : csv파일을 데이터프레임으로 변환한다. 별도의 옵션이 없으면
      첫번째 행은 컬럼명으로 간주한다. 그리고 행 인덱스는 정수형으로 지정된다. 
'''
...
'''
index _col : 행의 인덱스로 사용할 열의 번호를 지정한다. 
...
'''
c0컬럼을 인덱스로 지정한다. 그러면 인덱스로 지정된 컬럼은
...
'''
names 옵션으로 컬럼을 지정하는 경우 데이터의 개수보다 적으면 끝에서부터 
맞춰 적용된다. 즉 c0이 인덱스가 되고, c1~c3까지는 데이터가 된다. 
'''
...
'''
skiprows : 처음 몇개의 행을 건너뛴 후 데이터프레임으로 변환할지 설정한다.
'''
