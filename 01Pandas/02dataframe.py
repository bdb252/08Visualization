...
'''
데이터프레임(Data Frame)
    : 1차원 배열 형태인 시리즈가 여러개 모여있는 형태로 2차원 배열과
    동일하다. R의 데이터프레임을 착안하여 만들어졌다. Row와 Column으로 구성된다. 
'''
...
인덱스는 행이되고, 컬럼은 열로 지정된다. 
'''
df = pd.DataFrame([[20,'남','부산'], [17, '여', '서울']],
                  index = ['철수', '영희'], columns=['나이', '성별', '지역'])
print('데이터프레임2\n', df)
# 데이터프레임에서 인덱스와 컬럼만 리스트로 출력
print('index\n', df.index)
print('columns\n', df.columns)
print("="*30)

# 변경1 : 인덱스와 컬렴명의 속성을 통해 변경
df.index=['학생1', '학생2']
df.columns=['연령', '남녀', '거주']
print('변경1\n', df)
...
해당 옵션을 생략하면 False가 디폴트값이므로 새로운 복사본이 객체를 반환하게 된다. '''
...
# 문자형 인덱스가 부여되더라고 정수형 인덱스는 사용할 수 있다.
stu2 = df.iloc[1]
print('stu1과 stu2\n', stu1, stu2)
...
# 열(컬럼)에 대한 삭제이므로 축옵션을 1로 설정한다.
# 첫번째 실행에는 implace옵션이 없으므로 원본 객체에 적용되지 않는다.
